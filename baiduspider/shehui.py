...
class ShehuiNews(object):
	"""docstring for ShehuiNews"""
	def __init__(self,url):
		super(ShehuiNews, self).__init__()
		self.url = url
		self.codedetect = ''
		self.articlelink = []
	...
	def saveData(self,url,title,article):
		...
		filename ='./baidunews/shehui/' + result + '.txt'
	...
	def getArticleLink(self):
		...
		if self.codedetect != 'utf-8' and self.codedetect != 'gbk' and self.codedetect !='GB2312':
			...
			pattern = re.compile(r'href="http.*target.*mon.*</a>|href="http.*mon.*target.*</a>', re.M)
	...
	def getArticleFile(self):
		...
		print('get ShehuiNewsFile success')

	def getShehuiNews(self):
		self.getArticleLink()
		self.getArticleFile()
