...
class LadyNews(object):
	"""docstring for LadyNews"""
	def __init__(self,url):
		super(LadyNews, self).__init__()
		self.url = url
		self.codedetect = ''
		self.articlelink = []
	...
	def saveData(self,url,title,article):
		...
		filename ='./baidunews/lady/' + result + '.txt'
	...
	def getArticleLink(self):
		...
		if self.codedetect != 'utf-8' and self.codedetect != 'gbk' and self.codedetect !='GB2312':
			...
			pattern = re.compile(r'<a href="http:.+?</a><i>',re.M)	
	...
	def getArticleFile(self):
		...
		print('get LadyNewsFile success!!!')

	def getLadyNews(self):
		self.getArticleLink()
		self.getArticleFile()
