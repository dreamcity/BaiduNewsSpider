...
class FinanceNews(object):
	"""docstring for FinanceNews"""
	def __init__(self,url):
		super(FinanceNews, self).__init__()
		self.url = url
		self.codedetect = ''
		self.articlelink = []
	...
	def saveData(self,url,title,article):
		...
		filename ='./baidunews/finance/' + result + '.txt'
	...
	def getArticleLink(self):
		...
		if self.codedetect != 'utf-8' and self.codedetect != 'gbk' and self.codedetect !='GB2312':
			...
			pattern = re.compile(r'<a href="http.*mon="col.*target.*</a>|<a href="http.*target.*mon="col.*</a>',re.M)
	...
	def getArticleFile(self):
		...
		print('get FinanceNewsFile success!!!')
