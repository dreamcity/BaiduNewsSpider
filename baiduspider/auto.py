...
class AutoNews(object):
	"""docstring for AutoNews"""
	def __init__(self,url):
		super(AutoNews, self).__init__()
		self.url = url
		self.codedetect = ''
		self.articlelink = []
	...
	def saveData(self,url,title,article):
		...
		filename ='./baidunews/auto/' + result + '.txt'
	...
	def getArticleLink(self):
		...
		if self.codedetect != 'utf-8' and self.codedetect != 'gbk' and self.codedetect !='GB2312':
			...
			pattern = re.compile(r'href="http:.+?<span class="s">',re.M)
	...
	def getArticleFile(self):
		...
		print('get AutoNewsFile success')

	def getAutoNews(self):
		self.getArticleLink()
		self.getArticleFile()
